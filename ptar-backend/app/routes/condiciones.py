from fastapi import APIRouter, Depends, Query
from sqlalchemy.ext.asyncio import AsyncSession
from sqlalchemy import text
from pydantic import BaseModel, ConfigDict
from datetime import date
from typing import Optional
from app.database import get_db

router = APIRouter()

TURNO_MAP = {"manana": 2, "tarde": 3, "noche": 1}


def _parse_turno(s: str) -> int:
    return TURNO_MAP.get(s.lower().replace("ñ", "n"), 2)


# ─── Modelos de entrada ───────────────────────────────────────────────────────

class MbrUnidadIn(BaseModel):
    caudal_permeado:   Optional[float] = None
    tmp:               Optional[float] = None
    nivel_tmp:         Optional[str]   = None   # bajo | medio | alto
    purga:             bool            = False
    purga_min:         Optional[int]   = None
    recirculacion:     bool            = False
    recirculacion_min: Optional[int]   = None
    observaciones:     Optional[str]   = None


class CondicionesMbrIn(BaseModel):
    model_config = ConfigDict(extra='ignore')
    fecha:        Optional[date] = None
    turno:        str
    usuario:      Optional[str] = None
    mbr1:         MbrUnidadIn  = MbrUnidadIn()
    mbr2:         MbrUnidadIn  = MbrUnidadIn()
    observaciones: Optional[str] = None


class CondicionesRoIn(BaseModel):
    model_config = ConfigDict(extra='ignore')
    fecha:                Optional[date]  = None
    turno:                str
    usuario:              Optional[str]   = None
    p_entrada_e1:         Optional[float] = None
    p_salida_e1:          Optional[float] = None
    p_entrada_e2:         Optional[float] = None
    p_salida_e2:          Optional[float] = None
    q_permeado_e1:        Optional[float] = None
    q_permeado_e2:        Optional[float] = None
    q_rechazo_rotametro:  Optional[float] = None
    flujo_normalizado_e1: Optional[float] = None
    p_filtro_cartuchos:   Optional[float] = None
    p_f1:                 Optional[float] = None
    p_f2:                 Optional[float] = None
    p_f3:                 Optional[float] = None
    fecha_cip:            Optional[date]  = None
    observaciones:        Optional[str]   = None


class CondicionesPtapIn(BaseModel):
    model_config = ConfigDict(extra='ignore')
    fecha:                     Optional[date]  = None
    turno:                     str
    usuario:                   Optional[str]   = None
    tmp_pantalla:              Optional[float] = None
    tiempo_filtracion_min:     Optional[int]   = None
    tiempo_purga_clarif_min:   Optional[int]   = None
    frecuencia_purga_clarif_h: Optional[float] = None
    observaciones:             Optional[str]   = None


# ─── GET helpers ──────────────────────────────────────────────────────────────

@router.get("/ultima-mbr")
async def get_ultima_mbr(db: AsyncSession = Depends(get_db)):
    try:
        row = (await db.execute(text(
            "SELECT * FROM condiciones_mbr_turno ORDER BY fecha DESC, turno DESC LIMIT 1"
        ))).mappings().first()
    except Exception:
        return {}
    return dict(row) if row else {}


@router.get("/ultima-ro")
async def get_ultima_ro(db: AsyncSession = Depends(get_db)):
    try:
        row = (await db.execute(text(
            "SELECT * FROM condiciones_ro_turno ORDER BY fecha DESC, turno DESC LIMIT 1"
        ))).mappings().first()
    except Exception:
        return {}
    result = dict(row) if row else {}
    # Última fecha CIP independiente del turno
    try:
        cip = (await db.execute(text(
            "SELECT fecha_cip FROM condiciones_ro_turno WHERE fecha_cip IS NOT NULL ORDER BY fecha_cip DESC LIMIT 1"
        ))).mappings().first()
        result["ultima_cip"] = str(cip["fecha_cip"]) if cip else None
    except Exception:
        result["ultima_cip"] = None
    return result


@router.get("/ultima-ptap")
async def get_ultima_ptap(db: AsyncSession = Depends(get_db)):
    try:
        row = (await db.execute(text(
            "SELECT * FROM condiciones_ptap_turno ORDER BY fecha DESC, turno DESC LIMIT 1"
        ))).mappings().first()
    except Exception:
        return {}
    return dict(row) if row else {}


@router.get("/caudales-ro")
async def get_caudales_ro(
    fecha: str = Query(...),
    turno: str = Query(...),
    db: AsyncSession = Depends(get_db),
):
    """Caudales del turno activo registrados en F-02 Reactivos (operacion_ro_turno)."""
    turno_int = _parse_turno(turno)
    try:
        row = (await db.execute(text("""
            SELECT caudal_entrada_mh, caudal_salida_mh
            FROM operacion_ro_turno
            WHERE fecha = :fecha AND turno = :turno LIMIT 1
        """), {"fecha": fecha, "turno": turno_int})).mappings().first()
    except Exception:
        return {"caudal_entrada_mh": None, "caudal_salida_mh": None}
    return dict(row) if row else {"caudal_entrada_mh": None, "caudal_salida_mh": None}


@router.get("/caudales-ptap")
async def get_caudales_ptap(
    fecha: str = Query(...),
    turno: str = Query(...),
    db: AsyncSession = Depends(get_db),
):
    """Datos del turno activo desde F-02 Reactivos (operacion_ptap_turno)."""
    turno_int = _parse_turno(turno)
    try:
        row = (await db.execute(text("""
            SELECT caudal_entrada_mh, caudal_salida_mh,
                   manga_cambiada, manga_cantidad,
                   cebs_realizados, cebs_cantidad
            FROM operacion_ptap_turno
            WHERE fecha = :fecha AND turno = :turno LIMIT 1
        """), {"fecha": fecha, "turno": turno_int})).mappings().first()
    except Exception:
        return {}
    return dict(row) if row else {}


# ─── POST /mbr ───────────────────────────────────────────────────────────────

@router.post("/mbr")
async def post_mbr(body: CondicionesMbrIn, db: AsyncSession = Depends(get_db)):
    fecha  = body.fecha or date.today()
    turno  = _parse_turno(body.turno)
    m1, m2 = body.mbr1, body.mbr2

    await db.execute(text("""
        INSERT INTO condiciones_mbr_turno
            (fecha, turno, usuario,
             mbr1_caudal_permeado, mbr1_tmp, mbr1_nivel_tmp,
             mbr1_purga, mbr1_purga_min, mbr1_recirculacion, mbr1_recirculacion_min, mbr1_observaciones,
             mbr2_caudal_permeado, mbr2_tmp, mbr2_nivel_tmp,
             mbr2_purga, mbr2_purga_min, mbr2_recirculacion, mbr2_recirculacion_min, mbr2_observaciones,
             observaciones)
        VALUES
            (:fecha, :turno, :usuario,
             :m1cp, :m1tmp, :m1niv, :m1pu, :m1pum, :m1re, :m1rem, :m1obs,
             :m2cp, :m2tmp, :m2niv, :m2pu, :m2pum, :m2re, :m2rem, :m2obs,
             :obs)
        ON DUPLICATE KEY UPDATE
            usuario=:usuario,
            mbr1_caudal_permeado=:m1cp, mbr1_tmp=:m1tmp, mbr1_nivel_tmp=:m1niv,
            mbr1_purga=:m1pu, mbr1_purga_min=:m1pum,
            mbr1_recirculacion=:m1re, mbr1_recirculacion_min=:m1rem, mbr1_observaciones=:m1obs,
            mbr2_caudal_permeado=:m2cp, mbr2_tmp=:m2tmp, mbr2_nivel_tmp=:m2niv,
            mbr2_purga=:m2pu, mbr2_purga_min=:m2pum,
            mbr2_recirculacion=:m2re, mbr2_recirculacion_min=:m2rem, mbr2_observaciones=:m2obs,
            observaciones=:obs
    """), {
        "fecha": fecha, "turno": turno, "usuario": body.usuario,
        "m1cp": m1.caudal_permeado, "m1tmp": m1.tmp, "m1niv": m1.nivel_tmp,
        "m1pu": 1 if m1.purga else 0,
        "m1pum": m1.purga_min if m1.purga else None,
        "m1re": 1 if m1.recirculacion else 0,
        "m1rem": m1.recirculacion_min if m1.recirculacion else None,
        "m1obs": m1.observaciones,
        "m2cp": m2.caudal_permeado, "m2tmp": m2.tmp, "m2niv": m2.nivel_tmp,
        "m2pu": 1 if m2.purga else 0,
        "m2pum": m2.purga_min if m2.purga else None,
        "m2re": 1 if m2.recirculacion else 0,
        "m2rem": m2.recirculacion_min if m2.recirculacion else None,
        "m2obs": m2.observaciones,
        "obs": body.observaciones,
    })
    await db.commit()
    return {"ok": True}


# ─── POST /ro ─────────────────────────────────────────────────────────────────

@router.post("/ro")
async def post_ro(body: CondicionesRoIn, db: AsyncSession = Depends(get_db)):
    fecha = body.fecha or date.today()
    turno = _parse_turno(body.turno)

    await db.execute(text("""
        INSERT INTO condiciones_ro_turno
            (fecha, turno, usuario,
             p_entrada_e1, p_salida_e1, p_entrada_e2, p_salida_e2,
             q_permeado_e1, q_permeado_e2, q_rechazo_rotametro,
             flujo_normalizado_e1, p_filtro_cartuchos,
             p_f1, p_f2, p_f3, fecha_cip, observaciones)
        VALUES
            (:fecha, :turno, :usuario,
             :pe1, :ps1, :pe2, :ps2,
             :qe1, :qe2, :qrr,
             :fn1, :pfc, :pf1, :pf2, :pf3, :cip, :obs)
        ON DUPLICATE KEY UPDATE
            usuario=:usuario,
            p_entrada_e1=:pe1, p_salida_e1=:ps1, p_entrada_e2=:pe2, p_salida_e2=:ps2,
            q_permeado_e1=:qe1, q_permeado_e2=:qe2, q_rechazo_rotametro=:qrr,
            flujo_normalizado_e1=:fn1, p_filtro_cartuchos=:pfc,
            p_f1=:pf1, p_f2=:pf2, p_f3=:pf3,
            fecha_cip=COALESCE(:cip, fecha_cip),
            observaciones=:obs
    """), {
        "fecha": fecha, "turno": turno, "usuario": body.usuario,
        "pe1": body.p_entrada_e1, "ps1": body.p_salida_e1,
        "pe2": body.p_entrada_e2, "ps2": body.p_salida_e2,
        "qe1": body.q_permeado_e1, "qe2": body.q_permeado_e2,
        "qrr": body.q_rechazo_rotametro, "fn1": body.flujo_normalizado_e1,
        "pfc": body.p_filtro_cartuchos,
        "pf1": body.p_f1, "pf2": body.p_f2, "pf3": body.p_f3,
        "cip": body.fecha_cip, "obs": body.observaciones,
    })
    await db.commit()
    return {"ok": True}


# ─── POST /ptap ───────────────────────────────────────────────────────────────

@router.post("/ptap")
async def post_ptap(body: CondicionesPtapIn, db: AsyncSession = Depends(get_db)):
    fecha = body.fecha or date.today()
    turno = _parse_turno(body.turno)

    await db.execute(text("""
        INSERT INTO condiciones_ptap_turno
            (fecha, turno, usuario,
             tmp_pantalla, tiempo_filtracion_min,
             tiempo_purga_clarif_min, frecuencia_purga_clarif_h,
             observaciones)
        VALUES
            (:fecha, :turno, :usuario,
             :tmp, :tfilm, :tpurg, :fpurg, :obs)
        ON DUPLICATE KEY UPDATE
            usuario=:usuario,
            tmp_pantalla=:tmp, tiempo_filtracion_min=:tfilm,
            tiempo_purga_clarif_min=:tpurg, frecuencia_purga_clarif_h=:fpurg,
            observaciones=:obs
    """), {
        "fecha": fecha, "turno": turno, "usuario": body.usuario,
        "tmp": body.tmp_pantalla, "tfilm": body.tiempo_filtracion_min,
        "tpurg": body.tiempo_purga_clarif_min, "fpurg": body.frecuencia_purga_clarif_h,
        "obs": body.observaciones,
    })
    await db.commit()
    return {"ok": True}
